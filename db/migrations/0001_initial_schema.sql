-- Initial schema + dev seed data for the chatapp.
--
-- Ported from mysql-backup/rn_chatapp.sql when the migration runner was
-- introduced. The DROP TABLE IF EXISTS lines from the original dump are
-- removed: idempotency is handled by the schema_migrations table.
--
-- IMPORTANT for prod bootstrap on OCI HeatWave: this migration assumes the
-- DB is empty. The live OCI DB already has these three tables and real
-- user data, so before the first CI run you must record this migration as
-- already applied:
--     INSERT INTO schema_migrations (version) VALUES ('0001_initial_schema');
-- Otherwise the runner will try to CREATE TABLE existing tables and fail.
...
-- Dev seed data. Safe to apply against an empty DB only.
INSERT INTO `users` VALUES
    (1,  1134750038, 'Shehryar',   'Ahmed',  'sharyarahmed4567@gmail.com',    'Thomas',   'https://notjustdev-dummy.s3.us-east-2.amazonaws.com/avatars/1.jpg', 'Active Now'),
...
INSERT INTO `messages` VALUES
    (1,  1134750038, 288862539,  'Tesn messague',           1645380777),
